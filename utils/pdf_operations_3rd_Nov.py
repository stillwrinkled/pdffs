...
def url_to_pdf(url):
    ...
    # Display disclaimer message on the frontend
    flash(disclaimer)

    # Create a temporary file to store the PDF output
    with tempfile.NamedTemporaryFile(suffix=".pdf", delete=True) as temp_pdf:
        # Generate the PDF from the URL and save it to the temporary file
        pdfkit.from_url(url, temp_pdf.name, options=options)

        # Calculate the size of the generated PDF in MB
        pdf_size_mb = os.path.getsize(temp_pdf.name) / (1024 * 1024)  # Convert bytes to MB
        ...
        if compression_ratio:
            with tempfile.NamedTemporaryFile(suffix=".pdf", delete=True) as compressed_pdf:
                # Compress the file using Ghostscript
                compress_pdf(temp_pdf.name, compressed_pdf.name, compression_ratio)

                # Ensure the compressed file is read and added to output
                with open(compressed_pdf.name, 'rb') as f:
                    output.write(f.read())
        else:
            # No compression needed; read the original file into the output
